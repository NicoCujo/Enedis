...
def basic_indicators():    
    ...
    with col2:
        st.subheader('Consommation Electrique moyenne cumulée sur 24h')
...
def indicators_matrix():
    ...
    with col8:
        st.subheader("Consommation d'Electricité par Jours de Vacances")
        cols = ['Jours Fériés', 'Week-end', 'Vacances Scolaires']
        calendrier = st.selectbox('Sélectionner le calendrier', cols)
        if calendrier == 'Jours Fériés':
            dico_feries = {0: 'Journée Normale', 1: 'Jours Fériés'}
